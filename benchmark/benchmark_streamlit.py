    ...
    with st.spinner("🔄 Exécution du benchmark en cours..."):
        ...
        for idx, data in enumerate(instances):
            ...
            results.append({
                ...
                'n_prec': data['n_prec'],
                'n_res': data['n_res'],
                'n_nr': data.get('n_nr', 0),
                'makespan': mk if valid else None,
                'time': t,
                'status': 'HEURISTIC' if valid else 'INFEASIBLE',
                'start_times': start if valid else None,
                'solver': 'HEURISTIC'
            })
...
def highlight_status(val):
    if val == 'HEURISTIC': return 'background-color: #d4edda'
    if val == 'INFEASIBLE': return 'background-color: #f8d7da'
    return ''
...
if selected_data['status'] == 'INFEASIBLE':
    ...
    if show_gantt and selected_data['start_times']:
        ...
        solver = RCPSPSolver(selected_raw)
        ...
        for i in range(1, selected_raw['n_total'] + 1):
            gantt_data.append({
                'Tâche': f"Job {i}",
                'Début': starts[i],
                'Fin': starts[i] + selected_raw['durations'][i-1],
                'Durée': selected_raw['durations'][i-1],
                'Type': 'Béton' if i in selected_raw.get('concrete_tasks', []) else 'Standard'
            })
        
        df_gantt = pd.DataFrame(gantt_data)
        
        fig_gantt = px.timeline(
            df_gantt, x_start='Début', x_end='Fin', y='Tâche',
            color='Type', color_discrete_map={'Standard': '#3498db', 'Béton': '#e74c3c'},
        ...
        fig_gantt.update_layout(height=max(400, selected_raw['n_total'] * 15))
